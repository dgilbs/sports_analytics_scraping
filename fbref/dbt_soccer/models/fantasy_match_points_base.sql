...
tackles * fp.tackles_won_points as tackles_won_points,
case 
...
interceptions * fp.interception_points as interception_points,
case 
...
f_keep.saves * fp.save_points as save_points,
(pk_attempts - pk_goals) * fp.pk_missed_points as pk_missed_points,
case
    when cast(f_pass.passes_completed as float)/nullif(f_pass.passes_attempted, 0) >= 0.85 then fp.pass_completion_points
    else 0
...
blocks * fp.block_points as block_points,
...
left join soccer.nwsl_fantasy_players fant on fant.id = dp.id
